...
export interface StatsPanelSection {
...
export const useStatsPanelItems = (options: StatsPanelItemsOptions = {}) => {
  ...
  let statsPanelSections: StatsPanelSection[] = [
    {
      label: "Utilities",
      items: [
        {
  ...
  statsPanelSections = statsPanelSections.map((section) => ({
    ...section,
    items: section.items.map((item) => ({
      ...item,
      hidden: hiddenStats?.has(item.id) ?? false,
    })),
  }));

  return statsPanelSections;
};
